 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class restore_qtype_speakautograde_plugin extends restore_qtype_plugin {
    ...
    protected function define_question_plugin_structure() {
        ...
        // Add own qtype stuff.
        $name = 'speakautograde';
        $path = $this->get_pathfor('/speakautograde');
    ...
    /**
     * Process the qtype/speakautograde element
     */
    public function process_speakautograde($data) {
        global $DB;
        ...
        if ($this->get_mappingid('question_created', $oldquestionid)) {
            ...
            $newid = $DB->insert_record('qtype_speakautograde_options', $data);
            $this->set_mapping('qtype_speakautograde_options', $oldid, $newid);
        }
    }
    ...
    public static function define_decode_contents() {
        ...
        return array(
            new restore_decode_content('qtype_speakautograde_options', $fields, 'qtype_speakautograde')
        );
    }
    /**
     * When restoring old data, that does not have the speakautograde options information
     * in the XML, supply defaults.
     */
    protected function after_execute_question() {
        global $DB;

        // select all Speak (auto-grade) questions that have no options
        $sql = 'SELECT 1 FROM {qtype_speakautograde_options} options WHERE options.questionid = q.id';
        $sql = 'SELECT * FROM {question} q WHERE q.qtype = ?'." AND NOT EXISTS ($sql)";
        $questions = $DB->get_records_sql($sql, array('speakautograde'));

        foreach ($questions as $q) {
            $options = qtype_speakautograde::get_default_values($q->id, true);
            $DB->insert_record('qtype_speakautograde_options', $options);
        }
    }

    /**
     * Given one question_states record, return the answer
     * recoded pointing to all the restored stuff for speakautograde questions.
    ...
    public function after_restore_question() {
        ...
        $rs = $DB->get_recordset_sql('SELECT options.id, options.errorcmid
                                        FROM {qtype_speakautograde_options} options
        ...
        foreach ($rs as $option) {
            if ($cmid = $option->errorcmid) {
                ...
                $DB->set_field('qtype_speakautograde_options', 'errorcmid', $cmid, array('id' => $option->id));
            }
        }
        $rs->close();
    }
